...
This module verifies the ability to update model weights in place from
...
weights. The core testing logic is to build in-memory tensor payloads and
update the running server via `/update_weights_from_tensor`, then verify
module checksums are consistent with expected payload-applied checksums.

To strictly verify correctness, we compare SHA-256 checksums before and after
...
11 test cases in 2 classes;
two model pairs are tested locally, one in CI.

=============================================================================

Class 1: TestUpdateWeightsFromTensor                (10 tests) — API contract, checksum, error paths, and memory
Class 2: TestUpdateWeightsFromTensorTP              (1 test) — TP payload gather + rank-0 checksum

-----------------------------------------------------------------------------

Class 1: TestUpdateWeightsFromTensor

Validate the update_weights_from_tensor API contract, request/response shape,
default behavior, target-module routing, checksum verification, corrupted
payload failure behavior, repeated-update memory stability, and flattened-bucket
full-transformer restore.

All tests share one class-scoped server (same process, same in-memory weights).
Tests that require "base model then update" explicitly reset to base model first
so behavior is order-independent and updates are real, not no-ops.

  • test_update_weights_from_tensor_direct

    base model -> direct in-memory transformer payload update.
    Verifies after-update transformer checksum == expected checksum computed
    by applying the same payload over on-disk tensors.

  • test_update_weights_from_tensor_flattened_bucket

    Applies equivalent transformer update via `load_format="flattened_bucket"`.
    Verifies flattened-bucket checksum result matches expected direct-update
    checksum and non-targeted modules stay unchanged.

  • test_update_weights_specific_modules

    Sends module-scoped payload dict and targets only transformer.
    Verifies targeted module checksum matches expected payload result and
    non-targeted modules remain unchanged.

  • test_update_weights_missing_serialized_named_tensors

    Request body missing serialized payload; must fail (400, success=False).
    Verifies server remains healthy and checksum is unchanged.

  • test_update_weights_invalid_tp_payload_size

    Sends invalid `serialized_named_tensors` list size (not 1 and not tp_size);
    must fail (400, success=False). Verifies checksum is unchanged.

  • test_update_weights_default_target_transformer

    Omits `target_modules`; verifies default target is transformer and only
    transformer checksum changes to expected value.

  • test_update_weights_nonexistent_module

    target_modules includes unknown module; must fail (400, success=False)
    with "not found in pipeline" error.

  • test_corrupted_tensor_update_fails

    Sends invalid-shape transformer tensor payload; must fail
    (400, success=False) with partial-update failure message.

  • test_update_weights_from_tensor_memory_leak

    Repeats tensor updates and checks process-tree GPU memory does not grow
    beyond tolerance, guarding against persistent memory regressions.

  • test_update_weights_from_tensor_flattened_bucket_full_transformer

    Perturbs transformer first, then restores full transformer via
    flattened-bucket chunks. Verifies restored transformer checksum equals
    base model checksum.

-----------------------------------------------------------------------------

Class 2: TestUpdateWeightsFromTensorTP

Ensure tensor updates work correctly for tensor-parallel servers where each
rank provides serialized payloads gathered via gloo, and rank-0 checksum
matches the expected TP-applied checksum.

  • test_update_weights_from_tensor_tp_gloo_payload_e2e

    Builds per-rank payloads via gloo gather, updates transformer in TP mode,
    and verifies rank-0 transformer checksum matches expected rank-0 value
    derived from TP checksum reconstruction.
"""
